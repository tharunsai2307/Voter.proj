...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#090a0f] text-slate-100 flex flex-col font-sans select-none selection:bg-[#2979ff] selection:text-white">
      {/* Header */}
      <header className="border-b border-[#1b1e2c] bg-[#0c0d16] px-6 py-4 flex items-center justify-between sticky top-0 z-40 backdrop-blur-md bg-opacity-80">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight bg-gradient-to-r from-white via-slate-100 to-slate-400 bg-clip-text text-transparent">LiquiditySim</h1>
            <p className="text-xs text-slate-400">High-Performance Market Microstructure Simulator Telemetry</p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-3 bg-[#131522] border border-[#212437] px-4 py-1.5 rounded-full">
          {connected ? (
            <>
              ...
              <span className="text-xs font-semibold text-emerald-400 tracking-wide uppercase">Backend Connected</span>
            </>
          ) : (
            <>
              <WifiOff className="w-4 h-4 text-rose-500 animate-bounce" />
              <span className="text-xs font-semibold text-rose-500 tracking-wide uppercase">Connecting...</span>
            </>
          )}
        </div>
      </header>
        ...
        <AnimatePresence>
          {!connected && (
            <motion.div 
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="absolute inset-0 bg-[#090a0f]/90 backdrop-blur-md flex flex-col items-center justify-center z-30"
            >
              <div className="bg-[#121422] border border-[#23273e] p-8 rounded-2xl max-w-md w-full text-center shadow-2xl relative overflow-hidden">
                ...
                <h3 className="text-xl font-bold mb-2">Waiting for LiquiditySim Backend</h3>
                <p className="text-sm text-slate-400 mb-6">
                  Please start your C++ simulator dashboard mode by running:<br/>
                  <code className="block bg-[#05060b] text-blue-400 p-2.5 rounded-lg mt-3 text-xs font-mono border border-slate-900">
        ...
        {metrics && (
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              {/* Card: Orders */}
              <div className="bg-[#121422] border border-[#1e2133] rounded-xl p-5 relative overflow-hidden">
                ...
                <div className="space-y-2.5">
                  <div className="flex items-baseline justify-between">
                    ...
                    <span className="text-lg font-bold font-mono-custom text-blue-400">{formatNum(metrics.ordersProduced)}</span>
                  </div>
                  <div className="flex items-baseline justify-between">
                    <span className="text-sm text-slate-400">Consumed:</span>
                    <span className="text-lg font-bold font-mono-custom text-indigo-400">{formatNum(metrics.ordersConsumed)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-[#121422] border border-[#1e2133] rounded-xl p-5 relative overflow-hidden">
                ...
                <div className="space-y-2.5">
                  <div className="flex items-baseline justify-between">
                    ...
                    <span className="text-lg font-bold font-mono-custom text-emerald-400">{formatNum(metrics.tradesExecuted)}</span>
                  </div>
                  <div className="flex items-baseline justify-between">
                    <span className="text-sm text-slate-400">Throughput:</span>
                    <span className="text-lg font-bold font-mono-custom text-blue-400">
              ...
              <div className="bg-[#121422] border border-[#1e2133] rounded-xl p-5 relative overflow-hidden">
                ...
                <div className="space-y-1.5">
                  <div className="flex items-baseline justify-between">
                    ...
                    <span className="text-sm font-bold font-mono-custom text-slate-200">{metrics.averageLatencyMicros.toFixed(2)} µs</span>
                  </div>
                  <div className="flex items-baseline justify-between">
                    <span className="text-xs text-slate-400">P99 Percentile:</span>
                    <span className="text-sm font-bold font-mono-custom text-amber-400">{metrics.p99LatencyMicros.toFixed(2)} µs</span>
                  </div>
                  <div className="flex items-baseline justify-between">
                    <span className="text-xs text-slate-400">Max:</span>
                    <span className="text-sm font-bold font-mono-custom text-rose-400">{metrics.maxLatencyMicros.toFixed(2)} µs</span>
                  </div>
                </div>
              </div>

              {/* Card: Market Regime */}
              <div className="bg-[#121422] border border-[#1e2133] rounded-xl p-5 flex flex-col justify-between">
                <div>
                  ...
                  <div className={`text-center py-2.5 px-4 rounded-lg border font-bold text-sm tracking-wide ${getRegimeColor(metrics.regimeName)}`}>
                    {metrics.regimeName.toUpperCase()}
                  </div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              {/* Chart: Throughput */}
              <div className="bg-[#121422] border border-[#1e2133] p-5 rounded-2xl shadow-lg">
            ...
            <div className="bg-[#121422] border border-[#1e2133] rounded-2xl p-5">
              ...
              <div className="grid grid-cols-2 gap-4 text-center">
                <div className="bg-[#090a0f] border border-[#1f2235] p-4 rounded-xl">
                  ...
                  <span className={`text-xl font-bold font-mono-custom ${metrics.pushFailures > 0 ? 'text-rose-500' : 'text-slate-400'}`}>
                    {formatNum(metrics.pushFailures)}
                  </span>
                </div>
                <div className="bg-[#090a0f] border border-[#1f2235] p-4 rounded-xl">
                  <span className="text-xs text-slate-400 block mb-1">Queue Pop Failures (Queue Underruns)</span>
                  <span className="text-xl font-bold font-mono-custom text-slate-400">
      ...
      {/* Footer */}
      <footer className="border-t border-[#1b1e2c] bg-[#07080d] py-3 text-center text-xs text-slate-500">
        LiquiditySim Portfolio Asset Dashboard • C++20 Threaded Engine Bridge
      </footer>
    </div>
  );
}
